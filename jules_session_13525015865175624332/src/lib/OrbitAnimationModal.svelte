...
<div class="modal-backdrop" on:click={close} on:keydown={(e) => { if(e.key === 'Escape') close(); }} role="dialog" aria-modal="true" aria-labelledby="orbit-modal-title">
                ...
                {#if quantumOrbitals.length > 0}
                  <div class="orbitals-row">
                    ...
                    {#each quantumOrbitals as orb}
                      ...
                      <div class="orbital-cell" role="button" tabindex="0" on:keydown={(e) => { if(e.key === 'Enter' || e.key === ' ') { selectedOrbitalType = orb.type; activeTab = '3d'; } }} on:click={() => {selectedOrbitalType = orb.type; activeTab = '3d';}}>
                        {#if orb.type === 's'}
