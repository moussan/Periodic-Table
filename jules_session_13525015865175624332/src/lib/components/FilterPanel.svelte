<script lang="ts">
  ...
  function handlePropertyRangeToggle(property: string) {
    ...
    if (range.enabled && range.min && range.max) {
      updatePropertyRange(property as keyof import('../types/element').PropertyRanges, [parseFloat(range.min), parseFloat(range.max)]);
    } else {
      updatePropertyRange(property as keyof import('../types/element').PropertyRanges, null);
    }
  ...
  function handlePropertyRangeChange(property: string) {
    ...
    if (range.enabled && range.min && range.max) {
      ...
      if (!isNaN(min) && !isNaN(max) && min <= max) {
        updatePropertyRange(property as keyof import('../types/element').PropertyRanges, [min, max]);
